...
app.config['CHECKPOINT_PATH'] = 'Wav2Lip/checkpoints/wav2lip_gan.pth'
app.config['AUDIO_PATH'] = 'sample_data/input_audio.wav'
app.config['VIDEO_PATH'] = 'sample_data/input_audio.wav'
...
def choose_dir():
    if request.method == 'POST':
        ...
        return render_template('choose_dir.html', directories=directories, output_path=output_p, time=int(time.time()))
...
def select_mp3():
    ...
    if request.method == 'POST':
        ...
        sound = AudioSegment.from_mp3(selected_mp3)
        sound.export(wav_path, format='wav')
        
        return 'Selected MP3: {}'.format(wav_path)
    else:
...
def allowed_file(filename):
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in app.config['ALLOWED_EXTENSIONS']
def convert_mp3_towav(mp3_filename, wav_filename):
